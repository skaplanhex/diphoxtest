...
C
      ...
      SUBROUTINE polintcteq (XA,YA,N,X,Y,DY)
...
C                 Use Polint which allows x to be anywhere w.r.t. the grid
         ...
         Call polintcteq (XVpow(0), Fij(1), 4, ss, Fx, Dfx)
...
C                                                This is the highest x bin:
        ...
        Call polintcteq (XVpow(Nx-3), Upd(J1), 4, ss, Fx, Dfx)
...
C                         1st Q-bin, as well as extrapolation to lower Q
        Call polintcteq (TV(0), Fvec(1), 4, tt, ff, Dfq)

      ElseIf (JLq .GE. Nt-1) Then
C                         Last Q-bin, as well as extrapolation to higher Q
        Call polintcteq (TV(Nt-3), Fvec(1), 4, tt, ff, Dfq)
      Else
